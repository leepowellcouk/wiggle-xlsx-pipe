...
/**
 * Creates an event
 * Note: Each buffer is an single line array from the parsed xlsx file
 */
streams.createEvent = through(
  function (buf) {
    ...
    if (type === 'event') {
      ...
      console.log(chalk.grey('[x] ') + chalk.white(eventData[1]));

      curEvent = new Event(eventData);

      if (url !== undefined && url !== '' && url.match(/^http/)) {
        // Pause the stream while we fetch the event badge
        this.pause();
